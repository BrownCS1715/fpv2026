...
/-
# LoVe Demo 1-2: Types, Terms, Definitions
...
We'll do an example "type derivation" using these rules on the board in class.
This syntax won't come up that much this semester, but the typing rules certainly will.
If you dabble in the programming languages, type theory, or logic literature,
you'll see these sorts of judgments all over the place.
...
Some functions are defined with *structural* recursion.
In this pattern, the function is defined in terms of itself applied to a
"smaller" argument. "Smaller" means that the argument is a subterm of the original
argument, and this guarantees that the recursion will eventually terminate.

-/

/-- The function that computes `2^n`-/
def twoPow : ℕ → ℕ
| Nat.zero   => 1
...
which peel off exactly one constructor at a time. But it has infrastructure for
reducing things that appear more general to structural recursion. -/
